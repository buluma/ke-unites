	...
	/*	if (!empty ($this->category->image)) {
		...
		for($iM = 0; $iM < $totalImg; $iM++) {
			...
			if ($this->items[$iS]->overlib == 0) {
				$this->items[$iS]->ooverlibclass = array('class' => 'pg-image img img-responsive img-responsive2');
			} else {
				$this->items[$iS]->ooverlibclass = array('class' => 'pimo pg-image img img-responsive img-responsive2');
			}
		...
		if ($this->tmpl['responsive'] == 0 || $this->tmpl['diff_thumb_height'] > 0) {
			$wT = 'width';
			$hT = 'height';
