...
?>
<?php $this->view('views.connections.editassets'); ?>
<form action="<?php echo r2('index.php?ext=chronoforms&cont=connections'); ?>" method="post" name="admin_form" id="admin_form" class="ui form">
	...
	<div class="ui">
		...
		<button type="button" class="ui button compact blue icon labeled toolbar-button" data-fn="saveform" name="apply" data-url="<?php echo r2('index.php?ext=chronoforms&cont=connections&act=edit'); ?>">
		...
		<div class="ui grid">
			...
			<div class="four wide column">
				<?php
					...
					asort($functions_groups);
				?>
				
				<div class="ui fluid accordion styled draggable-list">
					<?php foreach($functions_groups as $kfg => $functions_group): ?>
					...
					<div class="content<?php if(empty($kfg)): ?> active<?php endif; ?>">
						...
						<div class="ui grid center aligned small functions-list">
							<?php foreach($functions_info as $fn_name => $fn_info): ?>
								<?php if($fn_info['group'] == $functions_group): ?>
								<div class="eight wide column draggable" data-type="function" data-private="<?php echo !empty($fn_info['private']) ? 1 : 0; ?>" data-info='<?php echo json_encode($fn_info); ?>' style="padding:5px;">
									<div class="ui segment tiny" style="padding:10px 3px;">
										<div class="ui header small"><i class="icon <?php echo $fn_info['icon']; ?> fitted"></i><?php echo $fn_info['title']; ?></div>
									</div>
								</div>
								<?php endif; ?>
							<?php endforeach; ?>
			...
			<div class="twelve wide column events-data">
				...
				<div class="ui form">
					<div class="ui action input fluid">
						<input type="text" placeholder="<?php el('Event name...'); ?>" name="new_event_name" id="new_event_name">
						<button type="button" id="add_new_event" class="ui button green compact disabled"><?php el('Add event'); ?></button>
					</div>
				</div>
			</div>
	...
	<div class="ui bottom attached tab segment" data-tab="sections">
		<div class="ui grid">
					...
					<div class="content <?php if(empty($kvg)): ?> active<?php endif; ?>">
						...
						<div class="ui grid center aligned small views-list">
							<?php foreach($views_info as $vw_name => $vw_info): ?>
								...
								<div class="eight wide column draggable" data-type="view" data-info='<?php echo json_encode($vw_info); ?>' style="padding:5px;">
									<div class="ui segment tiny" style="padding:10px 3px;">
			...
			<div class="twelve wide column sections-data">
				...
				<div class="ui form">
					<div class="ui action input fluid">
						<input type="text" placeholder="<?php el('Section name...'); ?>" name="new_section_name" id="new_section_name">
						<button type="button" id="add_new_section" class="ui button green compact disabled"><?php el('Add section'); ?></button>
					</div>
				</div>
			</div>
			...
			<div class="five wide column">
				<div class="ui vertical pointing menu fluid G2-tabs locales-list">
					<?php foreach($this->data['Connection']['locales'] as $locale_n => $locale): ?>
						<a class="blue item <?php if($locale_n == 'en_GB'): ?>active<?php endif; ?>" data-tab="locales-<?php echo $locale['name']; ?>">
