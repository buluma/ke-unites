...
class Tables extends \G2\L\Controller {
	var $models = array('\G2\A\M\Group');
	...
	function index(){
		...
		$this->Composer->paginate('Table', $Model);
	...
	function refresh(){
		$this->redirect(r2('index.php?ext=chronofc&cont=tables&act=build'.rp('table_name', $this->data).rp('gcb[]', $this->data('gcb.0'))));
	}
	...
	function delete(){
		...
		$this->redirect(r2('index.php?ext=chronofc&cont=tables&name='.$this->data('table_name')));
	}
	...
	function build(){
		...
		if($this->data('table_name') AND $this->data('save')){
			...
			if(!empty($fields)){
				...
				$this->redirect(r2('index.php?ext=chronofc&cont=connections&act=clear_cache'));
			}else{
				//new table, create it
				$rows = array();
				$pkey = '';
				...
				$this->redirect(r2('index.php?ext=chronofc&cont=connections'));
			}
		}
